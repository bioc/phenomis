...
#' @export
setMethod(
  "correcting", signature(x = "MultiAssayExperiment"),
  function(x,
...
#' @export
setMethod(
  "correcting", signature(x = "SummarizedExperiment"),
  function(x,
           method.vc = c("loess", "serrf")[1],
           reference.vc = c("pool", "sample")[1],
           loess_span.vn = 1,
    ...
    norm.mn <- .correcting(
      data.mn = t(SummarizedExperiment::assay(x)),
...
#' @export
setMethod(
  "correcting", signature(x = "MultiDataSet"),
  function(x,
           method.vc = c("loess", "serrf")[1],
           reference.vc = c("pool", "sample")[1],
           loess_span.vn = 1,
...
#' @export
setMethod(
  "correcting", signature(x = "ExpressionSet"),
  function(x,
           method.vc = c("loess", "serrf")[1],
           reference.vc = c("pool", "sample")[1],
           loess_span.vn = 1,
...
.batch_correct <- function(data.mn,
  ...
  ref_mean.vn <- apply(
    data.mn[samp.df[, "sampleType"] == reference.c, ], 2,
    function(feat.vn) {
  ...
  pool_extra.ml <- matrix(FALSE,
    nrow = 2, ncol = length(batch_data.ls),
    dimnames = list(
      c("first", "last"),
      names(batch_data.ls)
    )
  ...
  if (!all(c(pool_extra.ml))) {
    ...
    for (i in seq_along(pool_extra_batch.vi)) {
      message_missing.c <- paste(
        rownames(pool_extra.ml)[!pool_extra.ml[
          ,
  ...
  for (batch.c in names(batch_data.ls)) { ## processing each batch individually
    ...
    if (method.c == "loess") {
      ...
      batch_normalized.mn <- .serrf_pred(
        data.mn = batch_data.mn,
        ref.vi = batch_ref.vi,
        all.vi = batch_all.vi,
  ...
  if (method.c == "loess") {
    normalized.mn <- sweep(normalized.mn,
      MARGIN = 2, STATS = ref_mean.vn,
      FUN = "*"
    )
  }
...
.loess <- function(raw.vn, ref.vi, pred.vi, span.n) {
  if (length(ref.vi) < 5) {
    ...
    return(stats::predict(
      stats::loess(raw.vn[ref.vi] ~ ref.vi,
...
.serrf_pred <- function(data.mn,
  ...
  for (j in seq_len(ncol(data.mn))) {
    ...
    while (length(corvar_j.vi) < corvar.i) {
      corvar_j.vi <- intersect(
        ref_cor_ord.vi[seq_len(length.i)],
