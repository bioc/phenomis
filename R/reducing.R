...
#' @export
setMethod(
  "reducing", signature(x = "MultiAssayExperiment"),
  function(x,
...
#' @export
setMethod(
  "reducing", signature(x = "SummarizedExperiment"),
  function(x,
           cor_method.c = "pearson",
           cor_threshold.n = 0.9,
           rt_tol.n = 6,
...
#' @export
setMethod(
  "reducing", signature(x = "MultiDataSet"),
  function(x,
           cor_method.c = "pearson",
           cor_threshold.n = 0.9,
           rt_tol.n = 6,
...
#' @export
setMethod(
  "reducing", signature(x = "ExpressionSet"),
  function(x,
           cor_method.c = "pearson",
           cor_threshold.n = 0.9,
           rt_tol.n = 6,
...
.reducing <- function(data.mn, ## data (matrix of numerics;
  ...
  message(
    sum(feat.df[, "redund_is"]), " chemically redundant features (",
...
.mzdiff_db <- function() {
  ...
  mzdiff_db.df <- read.table(
    system.file("extdata/mzdiff_db_default.csv", # default file from acorf
      package = "phenomis"
    ),
    header = FALSE,
    quote = "",
  ...
  colnames(mzdiff_db.df) <- c("type", "losses_or_gains", "delta_mass")

  # mzdiff_db.df <- read.table(system.file("extdata/mzdiff_db.tsv", # extended list by Annelaure; information missing
  #                                        package = "phenomis"),
  #                            header = TRUE,
  #                            quote = "",
  #                            sep = "\t",
...
.corrt_mi <- function(feat.df,
  ...
  if (!(rt_colname.c %in% colnames(feat.df))) {
    ...
    stop(
      "The 'rt_colname.c' value '", rt_colname.c,
...
.corrtmz <- function(feat.df,
  ...
  if (!(mz_colname.c %in% colnames(feat.df))) {
    ...
    stop(
      "The 'mz_colname.c' value '", mz_colname.c,
      "' was not found in the columns from the variable metadata:\n",
      stop_colvar.c
    )
  }
  ...
  mzannot_upper.mc <- vapply(
    ...
    function(mzdiff.i) {
      ...
      paste(
        which(abs(c(
          mzdiff_db.df[, "delta_mass"],
          mzmin.n
        ) - mzdiff.n) <= mzdiff_tol.n),
        collapse = "_"
      )
    }, character(1)
  )
  ...
  corrtmz.mc <- matrix("",
    nrow = nrow(corrt.mi), ncol = ncol(corrt.mi),
...
.reduce_annot <- function(group.vi,
  ...
  for (group.i in seq_along(table(group.vi))) {
    ...
    ion_sel.c <- group_feat.vc[which.max(feat.df[
      group_feat.vc,
      "redund_samp_mean"
    ])]
      ...
      for (linked.c in setdiff(group_feat.vc, group_feat.c)) {
        ...
        if (annot.c != "") {
          ...
          if (length(annot_split.vc)) {
            annot.c <- paste(vapply(
              annot_split.vc,
              function(annot_split.c) {
                .reduce_annot_mz(
                  annot_split.c = annot_split.c,
                  mzdiff_db.df = mzdiff_db.df,
                  feat.df = feat.df,
                  linked.c = linked.c,
    ...
    for (linked.c in setdiff(group_feat.vc, ion_sel.c)) {
      ...
      if (annot.c != "") {
        ...
        if (sign.i < 0) {
          annot_split.vc <- annot_split.vc[mzdiff_db.df[
            as.numeric(annot_split.vc),
            "type"
          ] != "isotope"]
        }
