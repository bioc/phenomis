...
#' @export
setMethod(
  "transforming", signature(x = "MultiAssayExperiment"),
  function(x,
           method.vc = c("log2", "log10", "none")[1],
...
#' @export
setMethod(
  "transforming", signature(x = "SummarizedExperiment"),
  function(x,
           method.vc = c("log2", "log10", "none")[1],
           report.c = c("none", "interactive", "myfile.txt")[2]) {
    ...
    x@metadata[["transformed"]] <- method.vc
...
#' @export
setMethod(
  "transforming", signature(x = "MultiDataSet"),
  function(x,
           method.vc = c("log2", "log10", "none")[1],
           report.c = c("none", "interactive", "myfile.txt")[2]) {
    if (!(length(method.vc) %in% c(1, length(x)))) {
...
#' @export
setMethod(
  "transforming", signature(x = "ExpressionSet"),
  function(x,
           method.vc = c("log2", "log10", "none")[1],
           report.c = c("none", "interactive", "myfile.txt")[2]) {
    if (length(method.vc) > 1) {
...
.transforming <- function(data.mn, ## data (matrix of numerics; samples x variables)
  ...
  if (!(method.c %in% c("log2", "log10", "none"))) {
    stop("The transforming method should be either 'log2', 'log10',
         or 'none'")
  }
  ...
  if (data_na.i > 0 && verbose.l) {
    message(
      "Missing values in the 'dataMatrix': ", data_na.i,
  ...
  if (data_zero.i > 0 && verbose.l) {
    message(
      "Zero values in the 'dataMatrix': ", data_zero.i,
